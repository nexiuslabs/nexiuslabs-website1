...
export function BuildWithAI() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-text">
      {/* Hero Section */}
      <section className="min-h-screen flex flex-col items-center justify-center px-6 text-center bg-gradient-to-br from-surface via-background to-surface pt-20">
        <h1 className="text-4xl md:text-6xl font-extrabold leading-tight mb-4 text-primary animate-fade-in-up">
          Build With AI <span className="text-primary">for Non‑Coders</span>
        </h1>
        <p className="text-lg md:text-xl max-w-2xl mb-8 animate-fade-in-up-delay-1 text-muted">
        ...
        <div className="flex flex-col sm:flex-row gap-4 animate-fade-in-up-delay-2">
          {cohorts.map((c) => (
            <button
              ...
              className="w-full px-8 py-4 rounded-2xl shadow-xl text-lg bg-primary text-white hover:bg-primary-dark transition-colors font-display font-semibold tracking-wide"
            >
              Join {c.label} – SGD 180
            </button>
          ))}
        </div>
        <p className="mt-4 text-sm text-muted animate-fade-in-up-delay-3">Limited seats · PayNow after registration</p>
      </section>

      {/* Features Section */}
      <section className="py-16 bg-background px-6 max-w-6xl mx-auto">
        <div className="grid gap-8 md:grid-cols-3">
          {[
            ...
            <div key={title} className="rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 bg-surface border border-surface p-6">
      ...
      <section className="py-16 bg-surface px-6">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12 text-primary">Choose Your Cohort</h2>
          <div className="grid gap-12 md:grid-cols-2">
            {cohorts.map((cohort) => (
              <div key={cohort.slug} className="bg-background rounded-2xl shadow-lg p-8">
                <h3 className="text-2xl font-bold mb-6 text-primary">{cohort.label}</h3>
                <ol className="relative border-l-4 border-primary/30 pl-6 space-y-10">
                  {cohort.sessions.map((s, idx) => (
                    <li key={idx}>
                      <span className="absolute -left-3 top-0 bg-primary w-6 h-6 rounded-full"></span>
                      <h4 className="text-lg font-semibold mb-2 text-text">{s.title}</h4>
                ...
                <button
                  ...
                  className="mt-6 rounded-xl shadow px-6 py-2 bg-primary text-white hover:bg-primary-dark transition-colors font-medium"
                >
                  Reserve Seat
                </button>
              </div>
      ...
      {/* Venue */}
      <section className="py-12 bg-background px-6 text-center">
        <h2 className="text-2xl font-bold mb-4 text-text">Venue</h2>
      ...
      {/* Instructors */}
      <section className="py-16 bg-background px-6 max-w-6xl mx-auto">
        <h2 className="text-3xl font-bold text-center mb-12 text-text">Meet Your Instructors</h2>
        <div className="grid gap-8 md:grid-cols-2">
          {[
            ...
            <div key={t.name} className="rounded-2xl shadow-lg bg-surface border border-surface p-6">
              <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                ...
                <div className="flex flex-col gap-2 text-center sm:text-left">
                  <a href={t.link} className="text-xl font-semibold text-primary hover:underline inline-flex items-center justify-center sm:justify-start gap-2" target="_blank" rel="noopener noreferrer">
                    {t.name}
                    <ExternalLink className="h-4 w-4" />
                  </a>
                  <span className="text-sm text-muted">{t.role}</span>
                  <p className="text-muted leading-relaxed mt-2">{t.blurb}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* FAQ */}
      <section className="py-16 bg-surface px-6 max-w-4xl mx-auto rounded-2xl">
        ...
        <div className="space-y-6">
          {[
            ...
            <details key={q} className="group bg-background rounded-lg p-4 shadow-sm">
              <summary className="font-medium cursor-pointer select-none text-text">
                {q}
              </summary>
              <p className="pt-2 text-muted leading-relaxed group-open:animate-fadeIn">{a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      {/* CTA Footer */}
      <footer className="py-12 bg-background text-muted text-center flex flex-col items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          {cohorts.map((c) => (
            <button
              key={c.slug + "-footer"}
              onClick={() => handleJoinCohort(c.label)}
              className="w-full px-8 py-3 rounded-2xl font-medium bg-surface text-white hover:bg-surface transition-colors"
            >
              Join {c.label}
            </button>
          ))}
